...
<article>
    ...
    <ol start="{data[0].aya}" style="{olStyle}" dir="rtl">
        {#each sura as aya}
            ...
            }"><span class="text">{aya.text}</span>{#if aya.sajda}<span class="icon-sajda" title="Sajda">Sajda</span>{/if}<span class="icon-aya" aria-hidden="true" />{#if aya.ruku}<span class="icon-ruku" aria-label="Ruku" title="{'Ruku ' + aya.ruku}">{toArabicNumerals(aya.ruku)}</span>{/if}</li>
        {/each}
    </ol>
</article>
